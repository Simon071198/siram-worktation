...
import { apiLocationOnlineDeviceList,  allKameraLemasmil,
  allKameraOtmil, 
  allKameraOtmilByLocation} from 'renderer/services/api';

  const tokenItem = localStorage.getItem('token');
  const dataToken = tokenItem ? JSON.parse(tokenItem) : null;
  const token = dataToken.token;

const SidebarPlayback = (props:any) => {
  ...
  let [locationDeviceListOtmil, setLocationDeviceListOtmil] = useState([{nama_ruangan_otmil:'',kamera:[{kamera_id:'',nama_kamera:''}],ruangan_otmil_id:''}]);
  let [locationDeviceListLemasmil, setLocationDeviceListLemasmil] = useState([{nama_ruangan_otmil:'',kamera:[{kamera_id:'',nama_kamera:''}],ruangan_otmil_id:''}]);
        ...
        {locationDeviceListOtmil?.map((locDevice,index)=>(
               ...
               <details className="group">
                 <summary className="flex gap-2 items-center font-medium cursor-pointer list-none text-white">
                   ...
                   <span className='capitalize'>{locDevice.nama_ruangan_otmil}</span>
                 </summary>
                 ...
                 <div className="py-1 ml-[30px]">
                   <details className="groupChild">
                     <summary className="grid grid-cols-1 gap-2 items-center font-medium cursor-pointer list-none text-sm">
                      ...
                      <NavLink to={`/kamera-playback/${device.kamera_id}`} key={index}>
...
export default SidebarPlayback;
