...
const SidebarPlayback = (props: any) => {
  ...
  return (
    <div
      className={` h-screen bg-slate-600 translate-x-0 duration-300 ease-linear overflow-y-hidden ${
        sidebarKamera ? 'w-0 translate-x-0' : 'w-72.5 -translate-x-full'
      }`}
    >
      <div
        className={`grid grid-cols-1 gap-4 px-3 py-1 mt-4 ${
                  ...
                  <details className="groupChild">
                    <summary className="grid grid-cols-1 gap-2 items-center font-medium cursor-pointer list-none text-sm">
                      {locDevice.kamera?.map((device, index) => (
                        <NavLink
                          to={`/kamera-playback/${device.kamera_id}`}
                          key={index}
                        >
