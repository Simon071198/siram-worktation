...
const KategoriPerkaraList = () => {
  ...
  const handleSearchClick = async () => {
    try {
      const response = await apiReadKamera({
        filter: { nama_kamera: filter },
      });
      setPages(response.data.pagination.totalPages);
      setRows(response.data.pagination.totalRecords);
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await apiReadKamera(param);
      if (response.data.status !== 'OK') {
  ...
  const handleSubmitDelete = (params: Params) => {
    ...
    apiDeleteKamera(fetchParams).then((res) => {
      if (res.data.status === 'OK') {
        setModalDeleteOpen(false);
        apiReadKamera(fetchParams).then((res) => {
  ...
  const handleSubmitAdd = async (params: Params) => {
    ...
    apiCreateKamera(fetchParams).then((res) => {
      if (res.data.status === 'OK') {
        setModalDeleteOpen(false);
        apiReadKamera(fetchParams).then((res) => {
          setData(res.data.records);
  ...
  const handleSubmitEditKategotiPerkara = async (params: Params) => {
    ...
    apiUpdateKamera(fetchParams).then((res) => {
      if (res.data.status === 'OK') {
        setModalDeleteOpen(false);
        apiReadKamera(fetchParams).then((res) => {
          setData(res.data.records);
  ...
  return isLoading ? (
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      <div className="flex flex-col">
        <div className="grid grid-cols-5 rounded-t-md bg-gray-2 dark:bg-slate-600 sm:grid-cols-6   ">
          ...
          return (
            <div>
              <div className="grid grid-cols-5 rounded-sm bg-gray-2 dark:bg-meta-4 sm:grid-cols-6">
                ...
                ) : item.status_kamera === 'tidak-aktif' ? (
