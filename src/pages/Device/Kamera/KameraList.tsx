...
const KameraList = () => {
  ...
  const handleSearchClick = async () => {
    ...
    try {
      const response = await apiReadKamera(params);
      setPages(response.data.pagination.totalPages);
      setRows(response.data.pagination.totalRecords);
    ...
    <div className='container py-[16px]'>
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div className="flex justify-center w-full">
          <div className="mb-4 flex gap-2 items-center border-[1px] border-slate-800 px-4 py-2 rounded-md">
            <div className="w-full flex">
              ...
              />
              <select
                className="ml-2 w-3/6 text-sm rounded border border-stroke  dark:text-gray dark:bg-slate-800 py-1 pl-3 pr-4.5 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary"
                name="status_gateway"
                value={filterStatus}
        ...
        <div className="flex flex-col">
          <div className={`grid ${isOperator ? 'grid-cols-5' : 'grid-cols-6'} rounded-t-md bg-gray-2 dark:bg-slate-600`}   >
