...
const GelangList = () => {
  ...
  const handleSearchClick = async () => {
    try {
      const response = await apiReadGelang({
        filter: { nama_gelang: filter },
      });
      setPages(response.data.pagination.totalPages);
      setRows(response.data.pagination.totalRecords);
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await apiReadGelang(param);
      if (response.data.status !== 'OK') {
  ...
  const handleSubmitDelete = (params: Params) => {
    const fetchParams = {
      params: params,
      token: token,
    };
    apiDeleteGelang(fetchParams).then((res) => {
      if (res.data.status === 'OK') {
        setModalDeleteOpen(false);
        apiReadGelang(fetchParams).then((res) => {
  ...
  const handleSubmitAdd = async (params: Params) => {
    const fetchParams = {
      params:params,
      filter : '',
      token: token,
    };
    apiCreateGelang(fetchParams).then((res) => {
      if (res.data.status === 'OK') {
        setModalDeleteOpen(false);
        apiReadGelang(fetchParams).then((res) => {
          setData(res.data.records);
  ...
  const handleSubmitEditKategotiPerkara = async (params: Params) => {
    ...
    apiUpdateGelang(fetchParams).then((res) => {
      if (res.data.status === 'OK') {
        setModalDeleteOpen(false);
        apiReadGelang(fetchParams).then((res) => {
          setData(res.data.records);
  ...
  return isLoading ? (
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      <div className="flex flex-col">
        <div className="grid grid-cols-7 rounded-t-md capitalize bg-gray-2 dark:bg-slate-600 sm:grid-cols-7 ">
          ...
          return (
            <div>
              <div className="grid grid-cols-7 rounded-sm bg-gray-2 dark:bg-meta-4 sm:grid-cols-7">
                ...
                <div className="hidden items-center justify-center p-2.5 sm:flex xl:p-5 mr-2 flex-wrap lg:flex-nowrap gap-2">
                  <button
