...
import { apiReadKasus, apiReadPenyidikan, apiCreatePenyidikan } from '../../services/api';
...
export const AddPenyidikan = ({defaultValue, onSubmit, nomorPenyidikan, handleNext}: any) => {
  ...
  const handleChange = (e: any) => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const filter = resultKasus.filter((data: any) => !resultPenyidikan.some((dataresult: any) => dataresult.nomor_kasus === data.nomor_kasus));
  ...
  const validateForm = () => {
    ...
    for (const [key, value] of Object.entries(formState)) {
      ...
      if ((key === 'nrp_wbp' || key === 'saksi_id' || key === 'penyidikan_id' || key === 'saksi_id' || key === 'wbp_profile_id') && !value) {
        continue;
      }
    }
