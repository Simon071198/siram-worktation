...
const EventList = () => {
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await apiReadAllEvent(param);
  ...
  const handleSubmitDelete = (params: Params) => {
    ...
    apiDeleteAllEvent(fetchParams).then((res) => {
      if (res.data.status === 'OK') {
        setModalDeleteOpen(false);
        apiReadAllEvent(fetchParams).then((res) => {
  ...
  const handleSubmitAdd = (params: Params) => {
    const fetchParams = {
      params:params,
      token:token,
    };
    apiCreateAllEvent(fetchParams).then((res) => {
      if (res.data.status === 'OK') {
        setModalDeleteOpen(false);
        apiReadAllEvent(fetchParams).then((res) => {
          setData(res.data.records);
  ...
  const handleSubmitEdit = async (params: Params) => {
    const fetchParams = {
      params:params,
      token:token,
    };
    apiUpdateAllEvent(fetchParams).then((res) => {
      if (res.data.status === 'OK') {
        setModalDeleteOpen(false);
        apiReadAllEvent(fetchParams).then((res) => {
          setData(res.data.records);
          ...
          <div>
            <div className="grid grid-cols-5 rounded-sm bg-gray-2 dark:bg-meta-4 sm:grid-cols-5">
              ...
              <div className="flex items-center justify-center p-2.5 xl:p-5">
                <p className="text-meta-3">{item.waktu_mulai_kegiatan}</p>
              </div>
