...
const HakimList = () => {
  ...
  const handleFilterChange = async (e: any) => {
    ...
    setFilter(newFilter);
    // try {
    //   const response = await apiReadHakim({ filter: { nama: newFilter } });
  ...
  const handleSearchClick = async () => {
    try {
      ...
      const response = await apiReadHakim(params, token);
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await apiReadHakim(param, token);
      if (response.data.status !== 'OK') {
  ...
  const handleSubmitDeleteDataHakim = async (params: any) => {
    try {
      const responseDelete = await apiDeleteHakim(params, token);
  ...
  const handleSubmitAddDataHakim = async (params: any) => {
    console.log('DATA DARI LIST', params);
    try {
      const responseCreate = await apiCreateHakim(params, token);
  ...
  const handleSubmitEditDataHakim = async (params: any) => {
    console.log(params, 'edit');
    try {
      const responseEdit = await apiUpdateHakim(params, token);
  ...
  const exportToExcel = () => {
    const dataToExcel = [
      ['nip', 'nama hakim', 'alamat', 'departemen'],
      ...data.map((item: any) => [
        item.nip,
        item.nama_hakim,
        item.alamat,
        ...
        <div className="flex flex-col">
                ...
                return (
                  <div>
                    {isOperator ? (
                      ...
                      <>
                        <div
                          className="grid grid-cols-5 rounded-sm bg-gray-2 dark:bg-meta-4 capitalize"
                          key={item.nama_hakim}
                        >
                          <div
                            onClick={() => handleDetailClick(item)}
          ...
          {modalDetailOpen && (
            <AddHakimModal
              ...
              onSubmit={handleSubmitAddDataHakim}
          ...
          {modalEditOpen && (
            <AddHakimModal
              ...
              onSubmit={handleSubmitEditDataHakim}
          ...
          {modalDeleteOpen && (
            <DeleteHakimModal
              ...
              onSubmit={handleSubmitDeleteDataHakim}
