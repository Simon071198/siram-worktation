...
const dataAdmin = dataUserItem ? JSON.parse(dataUserItem) : null;
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await apiHakimRead(param,token);
      if (response.data.status !== 'OK') {
  ...
  const handleSubmitDeleteDataHakim = async (params: any) => {
    try {
      const responseDelete = await apiHakimDelete(params,token);
  ...
  const handleSubmitAddDataHakim = async (params: any) => {
    ...
    try{
      const responseCreate = await apiHakimInsert(params,token)
  ...
  const handleSubmitEditDataHakim = async (params: any) => {
    ...
    try {
       const responseEdit = await apiHakimUpdate(params,token)
